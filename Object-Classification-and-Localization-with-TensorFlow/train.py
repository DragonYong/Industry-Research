...
with tf.name_scope(name='conv2d_1'):
    ##CONVOLUTION LAYER 1
    # Weights for layer 1
...
with tf.name_scope(name='conv2d_4'):
    ##CONVOLUTION LAYER 4
    # Weights for layer 4
...
with tf.name_scope(name='conv2d_5'):
    ##CONVOLUTION LAYER 5
    # Weights for layer 5
...
with tf.name_scope(name='pool_3'):
    ##POOLING LAYER3
...
with tf.name_scope(name='dropout_1'):
    # Dropout Layer 1
...
with tf.name_scope(name='dropout_2'):
    # Dropout Layer 2
    hold_prob2 = tf.placeholder(tf.float32)
    s_fc2 = tf.nn.dropout(s_fc2, keep_prob=hold_prob1)
with tf.name_scope(name='fc_class_3'):
...
with tf.name_scope(name='fc_location_3'):
...
cross_entropy = tf.multiply(
    tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_true_1, logits=y_pred_1)), 10)
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(epochs):
            ...
            for v in range(0, len(cv_x) - int(len(cv_x) % validating_size), validating_size):
                acc_on_cv, loss_on_cv, preds, coordinates = sess.run(
                    [acc, cross_entropy, tf.nn.softmax(y_pred_1), y_pred_2],
                    feed_dict={x: cv_x[v:v + validating_size], y_true_1: cv_y1[v:v + validating_size],
                               y_true_2: cv_y2[v:v + validating_size],
        ...
        saver.save(sess, os.path.join(os.getcwd(), "ckpt/CNN_OL.ckpt"))
